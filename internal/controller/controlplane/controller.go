...
func (r *Reconciler) Reconcile(ctx context.Context, req types.NamespacedName) (Result, error) {
	logger := log.FromContext(ctx)
	logger.Info("Reconciling CAPTControlPlane")

	// Get CAPTControlPlane
	controlPlane := &controlplanev1beta1.CAPTControlPlane{}
	if err := r.Get(ctx, req, controlPlane); err != nil {
	...
	if err := r.Get(ctx, types.NamespacedName{
		...
		if apierrors.IsNotFound(err) {
			...
			_, setErr := r.setFailedStatus(ctx, controlPlane, cluster, "WorkspaceTemplateNotFound", fmt.Sprintf("Failed to get WorkspaceTemplate: %v", err))
			if setErr != nil {
				return Result{}, fmt.Errorf("failed to set status: %v (original error: %v)", setErr, err)
			}
			return Result{}, err
		}
		return Result{}, err
